    ...
    -->
        <app-table-title :page-type="pageType" :title="$tc('uncle.name', 2)" :has-pagination="hasPagination">
            <template v-slot:update>
                <notice-new-block page-id="uncle" @reload="resetFromUncle" />
            </template>
    ...
    -->
        <v-layout column fill-height class="pa-2">
            <v-flex v-if="!loading && !error" xs12>
                ...
                <v-card v-for="(uncle, index) in uncles" v-else :key="index" class="transparent" flat>
                    <table-uncles-row :uncle="uncle" :page-type="pageType" />
                </v-card>
                <v-layout v-if="hasPagination" justify-end class="pb-1 pt-2 pr-2 pl-2">
                    <app-paginate :total="pages" :current-page="page" @newPage="setPage" />
                </v-layout>
            </v-flex>
