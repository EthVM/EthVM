...
export function useSetPortfolio(hash: string) {
    ...
    const { newErc20Transfer, newETHTransfer, resetCount } = useAddressUpdate(addressHash, pause)
    ...
    const pauseUpdates = (_pause: boolean) => {
        pause.value = _pause
    }
    return { refetchBalance, setHash, pauseUpdates }
}
