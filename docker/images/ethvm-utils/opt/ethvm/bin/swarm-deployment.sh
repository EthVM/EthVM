#!/usr/bin/env bash

set -o errexit

install_swarm_prom() {
  git clone https://github.com/stefanprodan/swarmprom.git
  (cd swarmprom)
  ADMIN_USER=${admin_user} \
  ADMIN_PASSWORD=${admin_password} \
  SLACK_URL=https://hooks.slack.com/services/${slack_token} \
  SLACK_CHANNEL=${slack_channel} \
  SLACK_USER=${slack_user} \
  docker stack deploy -c docker-compose.yml swarmprom
}

#ethstats
install_eth_stats() {
  docker pull myetherwallet/docker-geth-lb:ethstats-instance
  docker service rm ethstats || true
  docker service create --name ethstats --env WS_SECRET=${ethstats_secret} --publish published=3030,target=3000 --replicas 1 myetherwallet/docker-geth-lb:ethstats-instance
}

#ethvm
chmod 777 /mnt/efs

deploy_ethvm() {
  local volumes=("zookeeper-1-volume" "zookeeper-2-volume" "zookeeper-3-volume" "kafka-1-volume" "kafka-2-volume" "kafka-3-volume" "kafka-connect-volume" )
  for volume in "$${volumes[@]}"; do
    mkdir -p /mnt/efs/$volume
  done

  local managers=("root-manager" "manager-1", "manager-2")
  for manager in "$${managers[@]}"; do
    docker node update --role manager $manager
  done

  local workers=("worker-1" "worker-2" "worker-3")
  for worker in "$${workers[@]}"; do
    docker node update --role worker $worker
  done

  docker stack deploy --compose-file /tmp/swarm-aws.yml ethvm
}

run() {
  local command="${1:-""}"
}
run "$@"
