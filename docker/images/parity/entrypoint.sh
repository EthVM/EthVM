...
case "$PARITY_CHAIN" in
    ...
    ;;
    private)
        $PARITY --config dev-insecure --reseal-min-period 10000 -lminer=trace --ws-apis all --tracing on --pruning archive --ws-interface 0.0.0.0 --ws-origins='*' --ws-hosts='*' --jsonrpc-interface 0.0.0.0 --jsonrpc-hosts='all' --jsonrpc-cors='all' --min-peers $PARITY_MIN_PEERS --max-peers $PARITY_MAX_PEERS
    ;;
    *)
