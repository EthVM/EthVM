...
function coerceInt(value) {
  if (value === '') {
    throw new TypeError('Int53 cannot represent non 53-bit signed integer value: (empty string)')
  }
  const num = Number(value)
  if (num !== num || num > MAX_INT53 || num < MIN_INT53) {
    throw new TypeError('Int53 cannot represent non 53-bit signed integer value: ' + String(value))
  }
  const int = Math.floor(num)
  if (int !== num) {
    throw new TypeError('Int53 cannot represent non-integer value: ' + String(value))
  }
  return int
}
