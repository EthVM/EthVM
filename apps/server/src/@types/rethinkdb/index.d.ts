...
declare module 'rethinkdb2' {
  ...
  export interface RArrayJoin<Left, Right> extends RArray<JoinResult<Left, Right>>, r.Zip<RArray<Left & Right>> {}

  export interface RStream<T>
    extends r.Run<CursorResult<T>>,
      r.Query,
      r.CoerceTo<T>,
      r.Changes<T>,
      r.OrderBy.Sequence<T>,
  ...
  export interface RStreamJoin<Left, Right> extends RStream<JoinResult<Left, Right>>, r.Zip<RStream<Left & Right>> {}

  export interface RSelection<T>
    extends r.Run<CursorResult<T>>,
      r.Operations<T>,
      r.Query,
      r.CoerceTo<T>,
  ...
  export interface RBase<T> extends r.Do, r.Query, r.CoerceTo<T>, r.ToJSON, r.Default<T>, r.Operators<T> {}

  export interface RValue<T> extends RBase<T>, r.Run<T> {}

  export interface RDatum<T>
    extends RValue<T>,
      r.Bracket.Datum<T>,
      r.GetField.Object,
      // Object.
  ...
  export interface RBoolean<T extends boolean> extends RValue<T>, r.And, r.Not, r.Or {}

  export interface RNumber<T extends number> extends RValue<T>, r.Add<r.NumberLike<T>>, r.Ceil, r.Div, r.Floor, r.Mod, r.Mul, r.Round, r.Sub<r.NumberLike<T>> {}

  export interface RString<T extends string> extends RValue<T>, r.Count.Datum, r.Add<r.StringLike<T>>, r.Downcase, r.Match, r.Split, r.Upcase {}
  ...
  export interface RSpecial<T extends 'ARGS' | 'ORDER_BY' | 'ERROR' | 'MAXVAL' | 'MINVAL' | 'LITERAL'> extends r.CoerceTo<T> {
    type: T
  }

  export interface RObject<T>
    extends RValue<T>,
      r.Bracket.Object,
      r.GetField.Object,
      r.Count.Datum,
  ...
  export interface RSelectionObject<T> extends RObject<T | null>, r.Operations<T>, r.Changes<T> {}

  export interface RSelectionObjectResult<T> extends RObject<T>, r.Operations<T>, r.Changes<T> {}
  ...
  export interface StatusResult {
    ...
    shards: Array<{
      ...
      replicas: Array<{ server: string; state: 'ready' | 'transitioning' | 'backfilling' | 'disconnected' | 'waiting_for_primary' | 'waiting_for_quorum' }>
    }>
  }
  ...
  export interface Connection extends EventEmitter {
    ...
    reconnect(options?: CloseOptions): Promise<Connection>
  ...
  export interface RTableSlice<T>
    extends r.Run<CursorResult<T>>,
      r.Query,
      r.Configurable,
      r.Operations<T>,
  ...
  export interface RBinary extends RValue<Buffer>, r.Count.Datum, r.Slice {}

  export interface RGeometry<T> extends RValue<T>, r.Intersects.Geometry, r.Includes.Geometry {
    /**
  ...
  namespace r {
    ...
    export interface Group<T> {
      ...
      group<Group>(
        field: GroupPredicate<T, Group>,
        field2: GroupPredicate<T, Group>,
        field3: GroupPredicate<T, Group>,
        options?: GroupOptions
      ): RGroupedStream<Group, T>
      group<Group>(
        field: GroupPredicate<T, Group>,
        field2: GroupPredicate<T, Group>,
        field3: GroupPredicate<T, Group>,
        field4: GroupPredicate<T, Group>,
        options?: GroupOptions
      ): RGroupedStream<Group, T>
    ...
    export interface Run<T> {
      ...
      run(connection: Connection, options?: RunOptions): Promise<T>
    }
    ...
    export interface Result<T> extends EventEmitter {
       ...
       */
      eachAsync(processFunction: (element: T, rowFinished?: (err: any) => void) => any, finalFunction?: (err?: Error) => any): Promise<void>
   ...
   * https://www.rethinkdb.com/api/javascript/union
   */
  export function union<T, Other>(
    stream: RStream<T>,
    sequence: r.SequenceLike<Other>,
    ...sequencesOrOptions: (r.SequenceLike<Other> | UnionOptions<T>)[]
  ): RStream<T | Other>
   ...
   */
  export function branch<T extends number>(
    test: r.BooleanLike<boolean>,
    trueAction: r.NumberLike<T>,
    test2: r.BooleanLike<boolean>,
    test2Action: r.NumberLike<T>,
    falseAction: r.NumberLike<T>
  ): RNumber<T>
  ...
  export function branch<T extends string>(
    test: r.BooleanLike<boolean>,
    trueAction: r.StringLike<T>,
    test2: r.BooleanLike<boolean>,
    test2Action: r.StringLike<T>,
    falseAction: r.StringLike<T>
  ): RString<T>
  ...
  export function branch<T extends boolean>(
    test: r.BooleanLike<boolean>,
    trueAction: r.BooleanLike<T>,
    test2: r.BooleanLike<boolean>,
    test2Action: r.BooleanLike<T>,
    falseAction: r.BooleanLike<T>
  ): RBoolean<T>
  ...
  export function branch<T>(
    test: r.BooleanLike<boolean>,
    trueAction: r.ArrayLike<T>,
    test2: r.BooleanLike<boolean>,
    test2Action: r.ArrayLike<T>,
    falseAction: r.ArrayLike<T>
  ): RArray<T>
  ...
  export function branch<T>(
    test: r.BooleanLike<boolean>,
    trueAction: T | RValue<T>,
    test2: r.BooleanLike<boolean>,
    test2Action: T | RValue<T>,
    falseAction: T | RValue<T>
  ): RDatum<T>
  ...
  export function branch<T extends RValue<U>, U>(
    test: r.BooleanLike<boolean>,
    trueAction: T | U,
    test2: r.BooleanLike<boolean>,
    test2Action: T | U,
  ...
  export function circle(
    longitudeAndlatitude: [r.NumberLike<number>, r.NumberLike<number>] | RArray<number>,
    radius: r.NumberLike<number>,
  ...
  export function map<T1, T2, Out>(
    sequence1: r.ArrayLike<T1>,
    sequence2: r.ArrayLike<T2>,
    mappingExpression: RValue<Out> | ((item1: RDatum<T1>, item2: RDatum<T2>) => RValue<Out> | Out)
  ): RArray<Out>
  export function map<T1, T2, T3, Out>(
    sequence1: r.ArrayLike<T1>,
    sequence2: r.ArrayLike<T2>,
    sequence3: r.ArrayLike<T3>,
    mappingExpression: RValue<Out> | ((item1: RDatum<T1>, item2: RDatum<T2>, item3: RDatum<T3>) => RValue<Out> | Out)
  ): RArray<Out>
  export function map<T1, T2, T3, T4, Out>(
    sequence1: r.ArrayLike<T1>,
    sequence2: r.ArrayLike<T2>,
    sequence3: r.ArrayLike<T3>,
    sequence4: r.ArrayLike<T4>,
    mappingExpression: RValue<Out> | ((item1: RDatum<T1>, item2: RDatum<T2>, item3: RDatum<T3>, item4: RDatum<T4>) => RValue<Out> | Out)
  ): RArray<Out>
  ...
  export function time(
    year: r.NumberLike<number>,
    month: r.NumberLike<number>,
    day: r.NumberLike<number>,
    hour: r.NumberLike<number>,
    minute: r.NumberLike<number>,
    second: r.NumberLike<number>,
    timezone?: r.StringLike<string>
  ): RTime
