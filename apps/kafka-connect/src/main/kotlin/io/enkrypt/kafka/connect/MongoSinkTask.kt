...
enum class CollectionId {
  ...
  Blocks, Accounts, Transactions, Contracts, FungibleBalances, NonFungibleBalances
...
class MongoSinkTask : SinkTask() {
  ...
  private fun processBlock(record: SinkRecord): Map<CollectionId, List<WriteModel<BsonDocument>>> {
    ...
    if (keySchema.type() != Schema.Type.STRUCT) throw IllegalArgumentException("Key schema must be a long")
  ...
  private fun processContractCreation(record: SinkRecord): Map<CollectionId, List<WriteModel<BsonDocument>>> {
    ...
    val keySchema = record.keySchema()
    if (keySchema.type() != Schema.Type.STRUCT) throw IllegalArgumentException("Key schema must be binary")
    ...
    if (record.value() == null) {
      ...
      val bson = StructToBsonConverter.convert(struct)
