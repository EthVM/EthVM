...
class Web3SourceConnector : SourceConnector() {
  ...
  override fun config(): ConfigDef = ConfigDef().apply {
    ...
    define(Config.WS_URL_CONFIG, ConfigDef.Type.STRING, Config.WS_URL_DEFAULT, ConfigDef.Importance.HIGH, Config.WS_URL_DOC)
    define(Config.TOPIC_BLOCKS_CONFIG, ConfigDef.Type.STRING, Config.TOPIC_BLOCKS_DEFAULT, ConfigDef.Importance.HIGH, Config.TOPIC_BLOCKS_DOC)
    define(Config.REPLAY_BUFFER_CONFIG, ConfigDef.Type.INT, Config.REPLAY_BUFFER_DEFAULT, ConfigDef.Importance.HIGH, Config.REPLAY_BUFFER_DOC)

    define(Config.SCHEMA_REGISTRY_URL_CONFIG, ConfigDef.Type.STRING, Config.SCHEMA_REGISTRY_URL_DEFAULT, ConfigDef.Importance.HIGH, Config.SCHEMA_REGISTRY_URL_CONFIG)
    define(Config.SCHEMA_BLOCK_KEY_ID_CONFIG, ConfigDef.Type.INT, null, ConfigDef.Importance.HIGH, Config.SCHEMA_BLOCK_KEY_ID_DOC)
    define(Config.SCHEMA_BLOCK_VALUE_ID_CONFIG, ConfigDef.Type.INT, null, ConfigDef.Importance.HIGH, Config.SCHEMA_BLOCK_VALUE_ID_DOC)
  }
  ...
  object Config {
    ...
    const val REPLAY_BUFFER_DOC = "A number which is subtracted from the last processed block number on startup before replay is initiated. Intended to ensure we don't miss any forks"
    ...
    fun replayBuffer(props: MutableMap<String, String>) = props.getOrDefault(REPLAY_BUFFER_CONFIG, REPLAY_BUFFER_DEFAULT).toInt()

    fun schemaRegistryUrl(props: MutableMap<String, String>) = props.getOrDefault(SCHEMA_REGISTRY_URL_CONFIG, SCHEMA_REGISTRY_URL_DEFAULT)
