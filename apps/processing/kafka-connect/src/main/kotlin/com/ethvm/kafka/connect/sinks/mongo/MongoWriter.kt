...
class MongoWriter {
  ...
  private var collections: Map<com.ethvm.kafka.connect.sinks.mongo.MongoCollections, MongoCollection<BsonDocument>> = emptyMap()
  ...
  fun addTopics(topics: Set<String>) {
    ...
    collections = collections + topics
      .map { com.ethvm.kafka.connect.sinks.mongo.MongoCollections.Companion.forTopic(it)!! }
  ...
  inner class BulkWriteTask(private val records: List<SinkRecord>) : Callable<Int> {
    ...
    override fun call(): Int {
      ...
      val batch = mutableMapOf<com.ethvm.kafka.connect.sinks.mongo.MongoCollections, List<WriteModel<BsonDocument>>>()

      logger.trace { "Processing ${records.size} records" }

      records.forEach { record ->
        (com.ethvm.kafka.connect.sinks.mongo.TopicWriteGenerators.Companion.forTopic(record.topic()))(record)
      ...
      batch
        ...
        .map { (collectionId, writes) -> Pair(collectionId, writes.chunked(com.ethvm.kafka.connect.sinks.mongo.MongoWriter.Companion.chunkSizeFor(collectionId))) }
        .forEach { pair ->
  ...
  companion object {
    ...
    private val chunkSizeMap = mapOf(
      com.ethvm.kafka.connect.sinks.mongo.MongoCollections.Blocks to 20
    )

    fun chunkSizeFor(collectionId: com.ethvm.kafka.connect.sinks.mongo.MongoCollections, defaultSize: Int = 100) =
      com.ethvm.kafka.connect.sinks.mongo.MongoWriter.Companion.chunkSizeMap.getOrDefault(collectionId, defaultSize)
  }
}
