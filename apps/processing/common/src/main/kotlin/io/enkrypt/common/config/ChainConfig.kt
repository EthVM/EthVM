...
interface NetConfig {
  ...
  fun chainConfigForBlock(blockNumber: Long): ChainConfig
...
class BaseNetConfig(configs: List<Pair<Long, ChainConfig>>) : NetConfig {
  ...
  private val blockNumbers = configs.map { it.first }
  ...
  override fun chainConfigForBlock(blockNumber: Long): ChainConfig {
    var idx = 0
