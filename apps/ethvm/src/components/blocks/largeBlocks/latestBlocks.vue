  ...
  <v-card color="white" flat class="pt-3 pr-4 pl-4 mt-0">
    <v-layout v-if="frameBlocks" row wrap align-center pb-1>
      ...
      <v-flex hidden-sm-and-down md4 order-xs2>
        <v-layout justify-end> <footnote :footnotes="footnote"></footnote> </v-layout>
      </v-flex>
    </v-layout>
    <v-layout v-else row wrap align-center pb-1>
      ...
      <v-flex hidden-sm-and-down md4 order-md2>
        <v-layout justify-center> <footnote :footnotes="footnote"></footnote> </v-layout>
      </v-flex>
          ...
          <v-card v-for="block in getBlocks" class="transparent" flat v-bind:key="block.hash">
            <v-layout grid-list-xs row wrap align-center justify-start fill-height pl-3 pr-2 pt-2 pb-1>
              ...
              <v-flex xs12 sm7 md6 lass="pr-0" order-xs3 order-sm2>
                <p class="text-truncate info--text  psmall mb-0 pb-2">
                  {{ $t('common.hash') }}:
              ...
              <v-flex xs12 sm7 md6>
                <v-card flat color="uncleGrey">
                  ...
                  <v-card-text v-for="(uncle, index) in block.getUncles" :key="index" class="text-truncate info--text"
                    >Hash:
                    <router-link :to="'/block/' + '0x' + uncle.unclesHash"> 0x{{ uncle.unclesHash }} </router-link>
                  </v-card-text>
                </v-card>
              </v-flex>
