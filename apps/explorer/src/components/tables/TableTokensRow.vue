       ...
       <v-flex xs6 sm2>
        ...
        <v-flex hidden-xs-only sm3>
          ...
          <v-tooltip v-if="getShortValue(getRoundNumber(balance(token.balance, token.decimals) * token.usdValue), true)" bottom>
            ...
            <span>${{ getShortValue(getRoundNumber(balance(token.balance, token.decimals) * token.usdValue), true) }} (@ ${{ getRoundNumber(token.usdValue) }} per {{ token.symbol }})</span>
          </v-tooltip>
          {{ getShortValue(getRoundNumber(balance(token.balance, token.decimals) * token. usdValue), false) }} (@ ${{ getRoundNumber(token.usdValue)}} per {{ token.symbol }})
        </h5>
        </v-flex>
     </v-layout>
  </router-link>
