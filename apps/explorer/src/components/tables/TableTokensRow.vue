<template>
  <router-link :to="'/token/' + token.address + '/holder=' + holder" v-if="token.balance != 0" class="tokens-data">
    <p class="token-symbol">{{ token.symbol }}</p> hello
    <p class="token-name">{{ token.name }}</p>
    <div class="token-balance">
      <div class>{{ getShortValue(balance(token.balance, token.decimals), false) }}</div>
      <div v-if="getShortValue(balance(token.balance, token.decimals), true)" class="tooltip-button token-tooltip" v-tooltip="balance(token.balance, token.decimals)">
        <i class="fa fa-question-circle-o" aria-hidden="true"></i>
      </div>
    </div>
    <div v-if="token.USDValue > 0" class="token-usd">
      <p>
        ${{ getRoundNumber(balance(token.balance, token.decimals) * token.USDValue) }} (@ ${{ getRoundNumber(token.USDValue) }} per {{ token.symbol }})
      </p>
    </div>
