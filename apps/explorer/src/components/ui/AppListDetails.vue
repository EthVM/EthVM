<template>
  <v-card color="white" flat class="pt-3">
    ...
    <div v-if="items">
          ...
          <v-layout align-center justify-start row fill-height class="pa-3">
            ...
            <v-flex xs1>
              <v-list-tile-action v-if="item.copy">
                <app-copy-to-clip :value-to-copy="item.detail"></app-copy-to-clip>
              </v-list-tile-action>
            </v-flex>
          </v-layout>
        </v-list-tile>
      ...
      <v-btn v-if="!more" @click="setView()" flat block class="secondary">
        <v-icon class="fa fa-angle-down white--text"></v-icon>
      </v-btn>
      <v-btn v-else @click="setView()" flat block class="secondary">
