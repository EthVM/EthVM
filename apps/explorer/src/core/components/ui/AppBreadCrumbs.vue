    ...
    <v-layout row wrap align-center justify-start class="pl-2 pr-2 mb-4">
      <v-card-title v-for="(item, i) in crumbs" :key="i" class="pl-0 pr-1 pb-0 text-truncate">
        ...
        <p v-if="item.link" class="pa-0 ma-0 text-truncate">
          <router-link v-if="!item.plural" :class="[item.disabled ? 'black--text' : 'info--text']" :to="item.link">{{ $t(item.text) }}{{ item.label }}</router-link>
          <router-link v-else :class="[item.disabled ? 'black--text' : 'info--text']" :to="item.link">{{ $tc(item.text, item.plural) }}{{ item.label }}</router-link>
        </p>
        <div v-else>
          <div v-if="!item.label">
