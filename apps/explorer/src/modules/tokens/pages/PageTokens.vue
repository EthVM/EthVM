    ...
    <v-card v-if="isRopsten" flat :class="{ 'pa-1': $vuetify.breakpoint.xsOnly, 'pa-3': $vuetify.breakpoint.smOnly, 'pa-5': $vuetify.breakpoint.mdAndUp }">
      <v-layout align-center justify-center column class="mb-4">
        ...
        <v-layout row>
          <v-spacer />
          <v-flex xs12 sm9 md7>
            <v-card-text class="font-weight-thin font-italic text-xs-center">{{ $t('message.ropsten-no-token.text') }}<a href="https://ethvm.com/tokens">{{ $t('message.ropsten-no-token.link') }} </a></v-card-text>
          </v-flex>
          <v-spacer />
        </v-layout>
      </v-layout>
