import { CalculateReward, SetBlock, SetTxReceipt, SetUncles, SetTraces, SetMinerBalances, SetStateDiff } from './processors/save-to-db'
    ...
    processBlockOld(blockNum, web3).then(blockOld => {
      blockOld.transactions.forEach((oldTx, idx) => {
        ...
        for (const address in oldStateDiff) {
          ...
          if (oldStateDiff[address].balance['+'] && oldStateDiff[address].balance['+'] === '0x0') continue
          if (oldStateDiff[address].balance['-'] && oldStateDiff[address].balance['-'] === '0x0') continue
          ...
          if (oldStateDiff[address].balance['+']) {
            ...
            assert(newStateDiff[address].balance['*'].to === oldStateDiff[address].balance['+'])
          } else if (oldStateDiff[address].balance['-']) {
            assert(newStateDiff[address].balance['*'].from === oldStateDiff[address].balance['-'])
            assert(newStateDiff[address].balance['*'].to === '0x0')
          } else {
            assert(newStateDiff[address].balance['*'].from === oldStateDiff[address].balance['*'].from)
            assert(newStateDiff[address].balance['*'].to === oldStateDiff[address].balance['*'].to)
          }
        }
      })
      runner(blockNum + 1)
