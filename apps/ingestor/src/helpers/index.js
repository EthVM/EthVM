...
const getGenesisState = () => {
  return require('ethereumjs-common/dist/genesisStates').genesisStateByName(
    Configs.CHAIN
  );
};
const getEthBalance = (address, blockNum, web3) => {
  if (utils.toBN(blockNum).lt(utils.toBN(0))) {
    ...
    if (genesis[address.toLowerCase()])
      return Promise.resolve(
        utils.toBN(genesis[address.toLowerCase()]).toString()
      );
  ...
  return new Promise((resolve, reject) => {
    if (utils.toBN(blockNum).lt(utils.toBN(0))) {
      ...
      addresses.forEach(address => {
        if (genesis[address.toLowerCase()])
          resultArr.push(utils.toBN(genesis[address.toLowerCase()]).toString());
