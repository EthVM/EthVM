...
const setBalanceChanges = (account, newBalance) => {
  ...
  account.diff = utils.toHex(
    utils.toBN(account.to).sub(utils.toBN(account.from))
  );
};
const cleanZeroBalances = changedBalances => {
  for (const address in changedBalances) {
    if (
      changedBalances[address].from === '0x0' &&
      changedBalances[address].to === '0x0'
    ) {
  ...
  getBalanceChangedAccounts() {
    ...
    _uncles.forEach(_uncle => {
      ...
      setBalanceChanges(
        ...
        utils
          .toBN(_uncle.reward)
          .add(utils.toBN(changedAccounts[uncleMiner].to))
      );
    });
