...
const consensus = {
  mainnet: block => {
    ...
    if (blockNumberBN.gte(utils.toBN(mainnet.hardforkBlock('byzantium')))) reward = BYZANTIUM_BLOCK_REWARD
    if (blockNumberBN.gte(utils.toBN(mainnet.hardforkBlock('constantinople')))) reward = CONSTANTINOPLE_BLOCK_REWARD
    block.uncles.forEach(ommer => {
      ommer.reward = utils.toHex(calculateOmmerReward(utils.toBN(ommer.number), blockNumberBN, reward))
    })
