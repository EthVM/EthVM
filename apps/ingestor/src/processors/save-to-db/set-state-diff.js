    ...
    return new Promise(resolve => {
      ...
      setInitialState(this.web3, block, prevBlockNum).then(state => {
        ...
        block.transactions.forEach(tx => {
          ...
          state[tx.from.toLowerCase()] = state[tx.from.toLowerCase()].sub(
            gasFee
          );
          ...
          transfers.forEach(transfer => {
            if (transfer.value !== '0x0') {
              state[transfer.from] = state[transfer.from].sub(
                utils.toBN(transfer.value)
              );
              state[transfer.to] = state[transfer.to].add(
                utils.toBN(transfer.value)
              );
              if (state[transfer.from].lt(utils.toBN(0)))
                throw new Error(
                  'balance went below 0: ' +
                    transfer.from +
                    ' ' +
                    block.number +
                    ' ' +
        ...
        if (verifyBalances) {
          ...
          addresses.forEach(address => {
            if (!utils.toBN(changes[address].from).eq(originalState[address]))
              throw new Error('balances dont match: previous:' + address);
          });
          getBatchEthBalance(addresses, block.number, this.web3).then(
            balances => {
              balances.forEach((balance, idx) => {
                if (
                  !utils
                    .toBN(changes[addresses[idx]].to)
                    .eq(utils.toBN(balance))
                )
                  throw new Error(
                    'balances dont match: destination:' +
                      addresses[idx] +
                      ' ' +
                      balance +
                      ' ' +
